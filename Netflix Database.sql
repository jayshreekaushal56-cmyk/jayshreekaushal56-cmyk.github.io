...
SELECT c.TitleName, SUM(v.Runtime) AS Total_Viewing_Hours
...
SELECT c.Category,
c.Genre,
SUM(v.Runtime) AS Total_Hours,
RANK() OVER (PARTITION BY c.Category 
ORDER BY SUM(v.Runtime) DESC) AS Genre_Rank
FROM viewinghistory v
JOIN content c ON v.ContentID = c.ContentID
...
SELECT p.PlanID, p.PlanName, COUNT(s.CustID) AS Subscriber_Count
FROM subscribes s
...
SELECT d.DeviceType, COUNT(u.ProfileID) AS Access_Count
FROM uses u
...
SELECT c.Category, AVG(v.Runtime) AS Avg_Viewing_Time
FROM viewinghistory v
JOIN content c ON v.ContentID = c.ContentID
GROUP BY c.Category;
 
-- 6. Identify the most preferred language by customers. Provide the number of customers, and language.
SELECT Language, COUNT(CustID) AS Customer_Count
...
SELECT c.TitleName, AVG(v.Runtime) AS Avg_Viewing_Time
FROM viewinghistory v
JOIN content c ON v.ContentID = c.ContentID
GROUP BY c.TitleName
...
SELECT c.CustID, CONCAT(c.FNAME, ' ', c.LNAME) AS Customer_Name, s.EndDate
FROM subscribes s
JOIN customers c ON s.CustID = c.CustID
WHERE YEAR(s.EndDate) >= 2028
ORDER BY s.EndDate DESC, Customer_Name;

-- 14. Display Average Revenue generated from each city. Rank city based on average revenue. 
SELECT City,AVG(PaymentAmount) AS Avg_Revenue, RANK() OVER (ORDER BY AVG(PaymentAmount) DESC) AS City_Rank

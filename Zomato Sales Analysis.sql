...
SELECT f.food_id, f.food_name, SUM(f.price_per_unit * ofd.quantity) AS total_revenue
FROM foods f
JOIN order_food ofd
ON f.food_id = ofd.food_id
GROUP BY f.food_id, f.food_name
ORDER BY total_revenue DESC;


-- 5. Find the second highest revenue-generating restaurant. 
SELECT r.restaurant_name, SUM(f.price_per_unit * ofd.quantity) AS total_revenue
FROM restaurant r
...
SELECT f.food_name, SUM(ofd.quantity) AS total_quantity
FROM foods f
...
SELECT MONTH(order_time) AS order_month, COUNT(order_id) AS total_orders
FROM order_detail
...
SELECT c.customer_id, c.customer_name, AVG(f.price_per_unit * ofd.quantity) AS avg_order_amount
FROM customer c
...
FROM (SELECT restaurant_id, customer_id, COUNT(order_id) AS total_orders, 
DENSE_RANK() OVER ( PARTITION BY restaurant_id ORDER BY COUNT(order_id) DESC) AS rnk
FROM order_detail
...
FROM (SELECT food_id,food_name,price_per_unit,
...
SELECT restaurant_id, COUNT(DISTINCT food_id) AS total_food_items
FROM order_food
...
SELECT p.payment_type, SUM(f.price_per_unit * ofd.quantity) AS total_payment
